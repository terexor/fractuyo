...
test.serial("creating note", tester => {
	...
	debitNote.setInvoiceDocumentReference(billingReference)
...
test.serial("signing note", async tester => {
	...
	// Remove this return to validate debit note with XSD
	return

	try {
		const mainXsdContent = fs.readFileSync("./tests/xsd/2.1/maindoc/UBL-DebitNote-2.1.xsd", "utf8").replace(
		...
		const xmlValidated = await debitNote.validateXmlWithXsd(
			{
				fileName: "UBL-DebitNote-2.1.xsd",
