...
test.serial("creating invoice", (tester) => {
	...
	invoice.setIssueDate(new Date("11-Feb-2026 UTC"))
	...
	// More than 700 to apply detraction
	product.setUnitValue(1000.00)
	product.calcMounts()

	invoice.addItem(product)

	invoice.setDetraction(12)
	invoice.getDetraction().calcAmount(invoice.taxInclusiveAmount)
	invoice.getDetraction().setCode("037")
	invoice.getDetraction().setFinancialAccount("14070123123")
	tester.true(invoice.hasDetraction())

	tester.is(customer.getIdentification().getNumber(), "20545314437")

	tester.is(invoice.getId(true), "01-F000-19970601")
	tester.is(invoice.getQrData(), "20606829265|01|F000|19970601|180.00|1180.00|2026-02-11|6|20545314437")
})
